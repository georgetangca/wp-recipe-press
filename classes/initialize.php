...
class recipePress_Init extends recipePressCore {
     ...
     function recipePress_Init() {
          ...
          //     add_action('admin_notices', array(&$this, 'upgrade_warning'));
               ...
               add_action('init', array(&$this, 'setup_ingredients')); //george remove the ingrident tag type 
               
               //George add here
              add_action('init', array(&$this, 'setup_instructions'));
...
<?php
     ...
     function create_post_type() {
          ...
          $labels = array(
               'name' => 'Recipes Plus', //george change  $this->options['plural-name'],
        ...
        /*  
          $recipe_args = array(
			...
			'show_ui' => true,
			'query_var' => true,
			'menu_icon' => get_stylesheet_directory_uri() . '/images/recipes.png',
			'rewrite' => true,
			'capability_type' => 'post',
                        'hierarchical' => false,
			'menu_position' => 5,
     ...
     /**
      * Setup instructions taxonomy.
      */
     function setup_instructions() {
         /*
         $labels = array(
               'name' => __('Instructions', 'recipe-press'),
               'singular_name' => __('Instruction', 'recipe-press'),
               'search_items' => __('Search Instructions', 'recipe-press'),
               'popular_items' => __('Popular Instructions', 'recipe-press'),
               'all_items' => __('All Instructions', 'recipe-press'),
               'parent_item' => __('Parent Instructon', 'recipe-press'),
               'edit_item' => __('Edit Instructon', 'recipe-press'),
               'update_item' => __('Update Instructon', 'recipe-press'),
               'add_new_item' => __('Add Instructon', 'recipe-press'),
               'new_item_name' => __('New Instructon', 'recipe-press'),
               'add_or_remove_items' => __('Add or remove Instructons', 'recipe-press'),
               'choose_from_most_used' => __('Choose from the most used Instructons', 'recipe-press'),
          );

          $args = array(
               'hierarchical' => false,
               'label' => __('Instructons', 'recipe-press'),
               'labels' => $labels,
               'public' => true,
               'show_ui' => true,
               'capabilities' => array(
         ...
         */   
          //register_taxonomy('recipe-instruction', array('recipe'), $args);
         // register_taxonomy('recipe-instruction', array('recipe'));
         // $this->taxonomy_rewrite_rules('recipe-instruction', array('slug' => $this->options['instruction-slug']));
     ...
     function init_metaboxes() {
          ...
          //George add here for instruction
        
          add_meta_box('recipe_child_flag_meta', __('Child Flag', 'recipe-press'), array(&$this,'recipe_child_flag'), 'recipe', 'normal', 'high' );
	 
          add_meta_box('recipe_children_meta', __('Children ID', 'recipe-press'), array(&$this,'recipe_children'), 'recipe', 'normal', 'high' );
	 
          add_meta_box('recipe_servings_meta', __('Servings', 'recipe-press'), array(&$this,'recipe_servings'), 'recipe', 'normal', 'high' );
	  add_meta_box('recipe_prep_meta', __('Prep Time','recipe-press'), array(&$this,'recipe_prep'), 'recipe', 'normal', 'high' );
	  
          //For addintional canadianfamily not this ite,  
          add_meta_box('recipe_cook_meta', __('Cook Time','recipe-press'), array(&$this,'recipe_cook'), 'recipe', 'normal', 'high' );
	 
          add_meta_box('recipes_instructions', __('Directions', 'recipe-press'), array(&$this, 'directions_box'), 'recipe', 'advanced', 'high');
          add_meta_box('recipes_ingredients', __('Ingredients', 'recipe-press'), array(&$this, 'ingredients_box'), 'recipe', 'advanced', 'high');
          add_meta_box('recipe_nutrients_meta', __('Nutrients Per Serving','recipe-press'), array(&$this,'recipe_nutrients'), 'recipe', 'normal', 'high' );
          
          
          add_meta_box("recipe_photo_credit-meta", __('Photo Credit','recipe-press'),array(&$this,'recipe_photo_credit'), 'recipe', 'normal', 'low');
          add_meta_box("recipe_subtitle-meta", __('Description','recipe-press'), array(&$this,'recipe_subtitle'), 'recipe', 'normal', 'low');
          add_meta_box("recipe_author-meta", __('Author','recipe-press'), array(&$this,'recipe_author'), 'recipe', 'normal', 'low');
         // add_meta_box("recipe_ratings_users-meta", __('Ratings Users','recipe-press'), array(&$this,'recipe_ratings_users'), 'recipe', 'normal', 'low');
         // add_meta_box("recipe_ratings_average-meta", __('Ratings Average','recipe-press'), array(&$this,'recipe_ratings_average'), 'recipe', 'normal', 'low');
         // add_meta_box("recipe_ratings_score-meta", __('Ratings Score','recipe-press'), array(&$this,'recipe_ratings_score'), 'recipe', 'normal', 'low');

  
          //George remove this for detail already in 
          //add_meta_box('recipes_details', __('Details', 'recipe-press'), array(&$this, 'details_box'), 'recipe', 'side', 'high');

          /* Display Nutritional value */
          //George remove here: can't be configurable through admin settting
          /*
      ...
      /**
      * Sets up the box for entering instructions.
      */
      //George add here 
     function recipe_child_flag() {
        global $post;
  	$custom = get_post_custom($post->ID);
  	$recipe_child_flag = $custom["_recipe_child_flag"][0]; /*$custom gets all fileds for the post, then $recipe_photo_credit gets just the photo credit fields*/
  	?>
  	<input type="radio" name="_recipe_child_flag" style="padding:5px;" value="0"  <?php if (empty($recipe_child_flag)){ ?> checked  <?php } ?>  
               onclick="javascript: document.getElementById('_recipe_children_area').style.display = '' ;"   />Normal recipe &nbsp;
        <input type="radio" name="_recipe_child_flag" style="padding:5px;" value="1"  <?php if (!empty($recipe_child_flag)){ ?> checked  <?php } ?>  
               onclick="javascript: document.getElementById('_recipe_children_area').style.display = 'none';" />Child recipe
        <?php
         
     }
         
     function directions_box() {
          /* Use nonce for verification */
          echo '<input type="hidden" name="instructions_noncename" id="instructions_noncename" value="' . wp_create_nonce('recipe_press_instructions') . '" />';
          include($this->pluginPath . 'includes/direction-form.php');
     }
     
     function recipe_children(){
  	global $post;
  	$custom = get_post_custom($post->ID);
      ...
      //  if(empty($recipe_child_flag)){
        ?>
        <div id="_recipe_children_area" style="<?php if (!empty($recipe_child_flag)) { ?> display:none; <?php } ?>">
  	<label>Children:</label>
  	<input type="text" id="_recipe_children_id" name="_recipe_children_id" style=" width:40%;" value="<?php echo $recipe_children; ?>" /><label> Notes:Use comma to differentiate multiple children</label>
        </div>
        <?php
       // }
    }
      
     function recipe_servings(){
  		global $post;
  		$custom = get_post_custom($post->ID);
  		$recipe_servings = $custom["_recipe_servings"][0]; /*$custom gets all fileds for the post, then $recipe_photo_credit gets just the photo credit fields*/
  	?>
  	<label>Servings:</label>
  	<input type="text" name="_recipe_servings" style="width:40%;" value="<?php echo $recipe_servings; ?>" />
  	<?php
       
      }
 
 	function recipe_prep(){
  		global $post;
  		$custom = get_post_custom($post->ID);
  		$recipe_prep = $custom["_recipe_prep"][0]; /*$custom gets all fileds for the post, then $recipe_photo_credit gets just the photo credit fields*/
  	?>
  	<label>Prep Time:</label>
  	<input type="text" name="_recipe_prep" style="width:40%;" value="<?php echo $recipe_prep; ?>" />
  	<?php
	}
        
        function recipe_cook(){
  		global $post;
  		$custom = get_post_custom($post->ID);
  		$recipe_cook = $custom["_recipe_cook"][0]; /*$custom gets all fileds for the post, then $recipe_photo_credit gets just the photo credit fields*/
  	?>
  	<label>Cook Time:</label>
  	<input type="text" name="_recipe_cook" style="width:40%;" value="<?php echo $recipe_cook; ?>" />
  	<?php
	}
 
        
        function recipe_nutrients(){
  		global $post;
  		$custom = get_post_custom($post->ID);
  		$recipe_nutrients = $custom["_recipe_nutrients"][0]; /*$custom gets all fileds for the post, then $recipe_photo_credit gets just the photo credit fields*/
  	?>
  	<label>Nutrients Per Serving:</label>
  	<input type="text" name="_recipe_nutrients" style="width:40%;" value="<?php echo $recipe_nutrients; ?>" />
  	<?php
	}
 
        
        function recipe_photo_credit(){
  		global $post;
  		$custom = get_post_custom($post->ID);
  		$recipe_photo_credit = $custom["_recipe_photo_credit"][0]; /*$custom gets all fileds for the post, then $recipe_photo_credit gets just the photo credit fields*/
  	?>
  	<label>Photo Credit:</label>
  	<input type="text" name="_recipe_photo_credit" style="width:40%;" value="<?php echo $recipe_photo_credit; ?>" />
  	<?php
	}
 
 	
 	function recipe_subtitle(){
  		global $post;
  		$custom = get_post_custom($post->ID);
  		$recipe_subtitle = $custom["_recipe_subtitle"][0];
  	?>
  	<label>Description:</label>
  	<input type="text" name="_recipe_subtitle" style="width:70%;" value="<?php echo $recipe_subtitle; ?>" />
  	<?php
	}
 
 	function recipe_author(){
  		global $post;
  		$custom = get_post_custom($post->ID);
  		$recipe_author = $custom["_recipe_author"][0];
  	?>
  	<label>Author:</label>
  	<input type="text" name="_recipe_author" style="width:40%;" value="<?php echo $recipe_author; ?>" />
  	<?php
	}
	
 	function recipe_ratings_users(){
  		global $post;
  		$custom = get_post_custom($post->ID);
  		$recipe_ratings_users = $custom["ratings_users"][0];
  	?>
  	<label>Ratings Users:</label>
  	<p><?php echo $recipe_ratings_users; ?></p>
  	<?php
	}
	
 	function recipe_ratings_average(){
  		global $post;
  		$custom = get_post_custom($post->ID);
  		$recipe_ratings_average = $custom["ratings_average"][0];
  	?>
  	<label>Ratings Average:</label>
  	<p><?php echo $recipe_ratings_average; ?></p>
  	<?php
	}	
	
	 function recipe_ratings_score(){
  		global $post;
  		$custom = get_post_custom($post->ID);
  		$recipe_ratings_score = $custom["ratings_score"][0];
  	?>
  	<label>Ratings Score:</label>
  	<p><?php echo $recipe_ratings_score; ?></p>
  	<?php	
	}
